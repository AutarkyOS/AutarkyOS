...
.DESCRIPTION
    ...
    a real UEFI implementation's USB stack. If autark's banner appears on the
...
$dpFile = Join-Path $env:TEMP ("autark-verify-{0}.txt" -f [guid]::NewGuid().ToString('N'))
...
if (Test-Path $log) {
    ...
    if ($text -match 'entered efi_main') {
        ...
        Write-Warning "autark did not start. The layout, not the laptop, is the problem."
