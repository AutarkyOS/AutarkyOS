...
## A benchmark is not a miner, and the BLAKE2s above was the benchmark

**This file arrives on `main` with a correction attached to it.** The table
above times `cuda/miner.cu`, which is a benchmark: its BLAKE2s hashes a single
64-byte block with the nonce at word 15. That is correct for what it computes,
verified against `hashlib`, and it is *not* the function a chain computes --
which is an 80-byte header across two compressions with a length counter, 64 on
the first block and 80 on the second. The two digests are unrelated. Shipping
the benchmark kernel as a miner would have produced wrong shares at a gigahash
a second, which `algo.cuh`'s own header names as the worst available outcome,
and what surfaced it was that its verify digest matched neither the header hash
nor the header's first block.

`cuda/xpu.cu` is the miner and does the two-block form. It also moves one thing
that the benchmark never had to think about: **the byte swap lives inside the
device, so the pipe carries protocol nonces both ways.** SHA-256 reads its
message big-endian and BLAKE2s little-endian, so the word each kernel indexes
by is a different view of the same four header bytes -- and a caller that has to
know which algorithms swap is a caller that gets it wrong for the third. Getting
it wrong is not an error but a share the pool rejects with nothing to say why.
Pinned by the strongest vector there is: given block 125552's header, `xpu.exe`
answers `9546a142`, which is that block's nonce as everybody quotes it, with no
swap asked of the caller.

Measured through the pool, on a **busy** host -- the pool daemon and the miner
on the same laptop, which is the condition this file says elsewhere costs
around a fifth:

| algorithm | over an 80-byte header, via `xpu.exe` |
|---|---|
| sha256d | 0.63 GH/s |
| blake2s | 1.28 GH/s |

The ratio holds at about 2x. The BLAKE2s figure is *higher* than the
single-block benchmark's 1.185 despite doing a second compression, which is the
thing worth not glossing over: the benchmark and the miner differ in launch
shape and in what they do with a hit as well as in the hash, so these are not
two measurements of one kernel and the pair should not be read as a speedup.
...
- **A third algorithm.** sha256d and blake2s are what `xpu.cu` has, so the
  field this card can join is narrower than the card is fast -- and sha256d is
  the one that is ASIC-dominated, which is most of why the second one was worth
  the day. kHeavyHash is measured above and is not wired to the miner.
